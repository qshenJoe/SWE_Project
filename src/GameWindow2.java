...
public class GameWindow2 extends JFrame implements ActionListener {
	...
	public GameWindow2() throws ClassNotFoundException, SQLException {
		...
		timer = new Timer(1000, new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (i <= 0) {
					...
					starSet[count - 1] = 0;
					String sql = "UPDATE Level_Two SET Time = ?, Star = ? WHERE Word_Seq = ?";
					try {
		...
		//TODO Auto-generated method stub
			if (e.getSource() == jbtConfirm) {
				if (!this.jtfInput.getText().equals("")) {
					...
					if(ans.equals(origin.toLowerCase()) && i > 15) {
						starSet[count - 1] = 3;
						String sql = "UPDATE Level_Two SET Time = ?, Star = ? WHERE Word_Seq = ?";
						try {
							stmt = con.prepareStatement(sql);
			...
			try {
				String sql = "SELECT * FROM Level_Two";
				stmt = con.prepareStatement(sql);
